...
describe('integration (nock)', () => {
  ...
  it('getGlobal returns the mocked payload', async () => {
    ...
    const client = new CoinpaprikaAPI({ fetcher: require('node-fetch') })
  ...
  it('apiKey sends Authorization: Bearer', async () => {
    nock(HOST, { reqheaders: { Authorization: 'Bearer secret-abc' } })
      .get('/v1/key/info')
      .reply(200, { plan: 'pro' })
    const client = new CoinpaprikaAPI({ apiKey: 'secret-abc', fetcher: require('node-fetch') })
  ...
  it('search bubbles up request rejection when host is unreachable', async () => {
    ...
    const client = new CoinpaprikaAPI({ fetcher: require('node-fetch') })
    await expect(client.search({ q: 'x' })).rejects.toThrow(/ENETUNREACH/)
  })

  it('abort via AbortSignal surfaces as a rejected promise', async () => {
    const controller = new AbortController()
    nock(HOST).get('/v1/global').delayConnection(200).reply(200, { ok: true })
    const client = new CoinpaprikaAPI({
      fetcher: require('node-fetch'),
